...
const transformValueSuitMappingToSequencialMapping = (valueMapping: CardValueMapping, suitMapping: SuitValueMapping): RevealedHand => {
    const entries = Object.entries(suitMapping).map(([suitIndex, suit]) => {
        const j = parseInt(suitIndex);
        return Object.entries(valueMapping).map(([valueIndex, value]) => {
            const i = parseInt(valueIndex);
            return { suit: j * 13 + i, value: j * 13 + i, suit_string: suit, value_string: value }
        })
    }).flat();
    return entries.reduce((acc, card) => ({ ...acc, [card.suit]: { suit: card.suit_string, value: card.value_string } }), {});
}
